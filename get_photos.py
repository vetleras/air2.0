...
def distance_calculation(latitude, longitude):
    ...
    max_latitude = np.max(
        [np.abs(point1_c[0] - latitude), np.abs(point2_c[0] - latitude)]
    )
    max_longitude = np.max(
        [np.abs(point1_c[1] - longitude), np.abs(point2_c[1] - longitude)]
    )
...
l = """?REQUEST=GetMap&BBOX=3238005,5039853,3244050,5045897&LAYERS=NATURAL-COLOR&MAXCC=20&WIDTH=320&HEIGHT=320&FORMAT=image/jpeg&TIME=2018-03-29/2018-05-29"""

def url(instance_id: str) -> str:
    return f"https://services.sentinel-hub.com/ogc/wms/{instance_id}" 
